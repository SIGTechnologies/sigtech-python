...
def test_wait_for_object_status(mocker):
    ...
    with pytest.raises(Exception) as e_info:
        mock_client.wait_for_object_status.return_value = Response({'status': 'FAILED', 'error': 'Test error'},
                                                                   'TestResponse')
        r.wait_for_object_status()
    assert str(e_info.value) == 'SigTech API Error - session_id : 456 - object_id : 123 - Message : Test error'
